...
with conn:
    with conn.cursor() as cur:
        sql = "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE table_schema = 'sys' AND table_name = 'github_db_2011_2021'"  # 테이블의 컬럼명을 가져온다.
        cur.execute(sql)
        result = cur.fetchall()
        ...
        while(i < 50000):
            ...
            try:
                ...
                for j in range(0, len(RepositoryData)):
                    ...
                    year = int(year)
                    if(year >= 2011 and year < 2022):  #2011년부터 2021년까지의 데이터만 저장
                        ...
                        if(LanguageKey == []):  #해당 레포지스토리에 언어가 있는지 확인
                            ...
                            sql = "INSERT INTO github_db_2011_2021 (User_ID, User_Name, Repository_Name, created_at, updated_at, pushed_at) VALUE(%s, %s,%s,%s,%s, %s)"
                            ...
                            for k in range(0, len(LanguageKey)):                        
                                if LanguageKey[k] not in LanguageList:  #해당 이름의 칼럼이 리스트에 존재하지 않는다면
                                    if(str.isalnum(LanguageKey[k]) == False):  #C++과 같은 특수문자인지 검사
                                        sql = "ALTER TABLE github_db_2011_2021 add `" + LanguageKey[k] + "` int NOT NULL default '0'"
                                    else:
                                        sql = "ALTER TABLE github_db_2011_2021 add " + LanguageKey[k] + " int NOT NULL default '0'"
                                    cur.execute(sql)
                                    conn.commit()
                                ...
                                if(str.isalnum(LanguageKey[k]) == False):
                                    sql = "UPDATE github_db_2011_2021 SET `" + LanguageKey[k] + "`=%s WHERE User_Name=%s AND Repository_Name=%s"
                                else:
                                    sql = "UPDATE github_db_2011_2021 SET " + LanguageKey[k] + "=%s WHERE User_Name=%s AND Repository_Name=%s"
                        ...
                        print(str(i) + " | " + owner + "의 " +RepositoryName +"가 " + str(year) + "년도여서 다음 레포지스토리를 탐색합니다.")
                i = i + 1
            except:
